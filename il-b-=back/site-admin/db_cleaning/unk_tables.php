...
foreach ($unk_tables as $table_name){
	if(true){
		$sql = "SELECT count(*) as del_count  FROM $table_name WHERE unk IN(".$unk_list.")";
		$res = mysql_db_query(DB,$sql);
		$count_arr = mysql_fetch_array($res);
		echo $table_name.":".$count_arr['del_count']."</br><br/>";
	}
	$c++;
}

$sql = "SELECT count(*) as del_count  FROM user_lead_sent WHERE sendToUnk IN(".$unk_list.")";
$res = mysql_db_query(DB,$sql);
$count_arr = mysql_fetch_array($res);
echo "user_lead_sent".":".$count_arr['del_count']."<br/>";
...
foreach ($uid_tables as $table_name){
	if(true){
		$sql = "SELECT count(*) as del_count  FROM $table_name WHERE user_id IN(".$uid_list.")";
		$res = mysql_db_query(DB,$sql);
		$count_arr = mysql_fetch_array($res);
		echo $table_name.":".$count_arr['del_count']."<br/>";
	}
	$c++;
}
...
foreach ($uid_type_tables as $table_name){
		$sql = "SELECT count(*) as del_count  FROM $table_name WHERE user_id IN(".$uid_list.") AND user_type = 'users'";
		$res = mysql_db_query(DB,$sql);
		$count_arr = mysql_fetch_array($res);
		echo $table_name.":".$count_arr['del_count']."<br/>";
	$c++;
...
foreach ($unk_tables_with_sub as $table_name=>$table_details){
		...
		foreach($table_details as $table_sub_name=>$table_sub_foreignkey){
			$sql_sub_2 = "SELECT count(*) as del_count  FROM ".$table_sub_name." WHERE ".$table_sub_foreignkey." IN (".$sub_id_list.")";
			
			$res_sub_2 = mysql_db_query(DB,$sql_sub_2);
			$data_sub_2 = mysql_fetch_array($res_sub_2);
			echo "----".$table_sub_name.":".$data_sub_2['del_count']."<br/>";
		}
		$sql_sub = "SELECT count(*) as del_count  FROM $table_name WHERE unk IN(".$unk_list.")";
		$res_sub = mysql_db_query(DB,$sql_sub);
		$data_sub = mysql_fetch_array($res_sub);
		echo $table_name.":".$data_sub['del_count']."<br/>";
}
...
foreach($product_id_tables as $table_name){
		$sql = "SELECT count(*) as del_count  FROM $table_name WHERE product_id IN(".$product_id_list.")";
...
$sql_products = "SELECT count(*) as del_count  FROM user_products WHERE unk IN(".$unk_list.")";
$res_products = mysql_db_query(DB,$sql_products);

echo "<br/>----------------<br/><br/>";
echo "user_products".":".$count_arr['del_count']."<br/>";
echo "<br/>----------------<br/><br/>";

$sql = "SELECT count(*) as del_count  FROM user_model_cat_belong WHERE itemId IN(".$product_id_list.") AND model = 'products'";
$res = mysql_db_query(DB,$sql);
$count_arr = mysql_fetch_array($res);
echo "user_model_cat_belong".":".$count_arr['del_count']."<br/>";
...
foreach($user_id_with_type_tables as $table_name){
		$sql = "SELECT count(*) as del_count  FROM $table_name WHERE user_id IN(".$product_id_list.") AND user_type='users'";
