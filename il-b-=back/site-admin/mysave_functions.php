...
function user_lead_settings()
{
	...
	echo "<table class=\"maintext\">";
		echo "<tr>";
			echo "<td colspan=1><A href=\"?main=user_profile&unk=".$_GET['unk']."&record_id=".$_GET['record_id']."&sesid=".SESID."\" class=\"maintext\">חזרה לעדכון</a></td>";
			echo "<td colspan=1><A href=\"?main=private_contacts_imports&unk=".$_GET['unk']."&sesid=".SESID."\" class=\"maintext\">ייבוא רשימות צור קשר</a></td>";
		echo "</tr>";
		echo "<tr>";
		...
		echo "<tr>";
			echo "<td width=50% valign=top>";
				...
				echo "<table class='maintext'>";
					...
					echo "</tr>";
					echo "<tr>";
						echo "<td><b style='color:red;'>הצג לקוח בטבלת שיחות שלא הפכו לליד</b></td>";
						echo "<td>";
					...
					echo "</tr>";
					echo "<tr>";
						echo "<td><b style='color:red;'>אפשרות להורדת הקלטות שיחה</b></td>";
						echo "<td>";
					...
					echo "<tr>";
						echo "<td><b style='color:red;'>שליחת התראות על עדכון לידים</b></td>";
						echo "<td>";
...
function leadsUserGet_client_csv()
{
	...
	$tagin_arr = array('0'=>array('id'=>'0','user_id'=>$uid,'tag_name'=>'ללא תיוג'));
  ...
  // SQL query that get by the user unk - the user contact forms between the date of the date right now -1 , and the the day before a month.
	...
	foreach($form_leads_paybypswd_arr as $lead){
		...
		if(!isset($_GET['withouthtml'])){
			$lead_row[] = "<a target='_BLANK' href='?main=sentLeadsAll&sesid=".SESID."&unk=".$_GET['unk_to_see']."&send_refund_request=".$lead['id']."'>שלח בקשה לזיכוי</a>";
		}
		if($advanced_report){
			...
			if(!empty($lead['previous_sends'])){
				...
				foreach($lead['previous_sends'] as $prev_lead){
					...
					if($prev_lead['lead_recource'] == 'form'){
						$prev_lead_row = array( stripslashes($prev_lead['date_in']) , stripslashes($prev_lead['name']) , stripslashes($prev_lead['email']) ,$billed_str, $prev_lead['opened_str']  ,'טופס באתר',$prev_lead['refunded_str'] , stripslashes($prev_lead['content']));
	...
	foreach($form_leads_doubled_arr as $lead){
		$row_leads_arr[] = array( stripslashes($lead['date_in']) , stripslashes($lead['name']) , stripslashes($lead['email']) , stripslashes($lead['phone']), $lead['opened_str'],$status_options[$lead['status']], $tagin_arr[$lead['tag']]['tag_name']  , 'טופס באתר',$lead['refunded_str'] , stripslashes($lead['content']));
	}
	...
	if(!isset($_GET['withouthtml'])){
		...
		$qstr .= "&phones_only=1";
		echo "<div><a href='$qstr'>לחץ כאן לצפות במספרי הטלפון בלבד</a></div>";		
...
function leadsUserGet_client_csv_phones_only()
{
  ...
  // SQL query that get by the user unk - the user contact forms between the date of the date right now -1 , and the the day before a month.
	...
	if(!isset($_GET['withouthtml'])){
		...
		echo "<div><a href='$qstr'>חזור לדוח המקורי</a></div>";
		echo "<table border=1 style='direction:rtl; text-align:right; border-collapse: collapse;'>";
		$max_cols = 0;
		foreach($row_leads_arr as $row){
			$cols_count = count($row);
...
function sentLeadsAll()
{
	...
	if(isset($_GET['send_refund_request'])){
		if(isset($_POST['refund_row_id'])){
			...
			echo "<a href='?main=view_estimate_form_refund_list&sesid=".SESID."'>המשך אל מערכת הזיכויים</a>&nbsp&nbsp";
			return;
		}
		$refund_request_form = "	
