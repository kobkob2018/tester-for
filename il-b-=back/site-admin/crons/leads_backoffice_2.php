...
$allLeadsSend = array();
$response = file_get_contents("http://212.143.60.5/reports/export_cdr.php?date=".date("Y-m-d",strtotime("yesterday")),false,$sslContext);
...
$checkSql = "SELECT uniqueid FROM sites_leads_check WHERE uniqueid IN(".$leads_sql_in.")";
        ...
        foreach ($leads as $key => $val) {
            if ($key == 'dst') {
                ...
                if (isset($users) && ! empty($users)) {
                    ...
                    $sql = "insert into sites_leads_check (unk, call_from, call_to, answer, sms_send, call_date,billsec,uniqueid,recordingfile) values('$unk', '$src', '$dst', '$answer', '$sms_send', '$call_date','$billsec','$uniqueid','$recordingfile')";
                    $res = mysql_db_query($db, $sql);
