...
if( $_SERVER[HTTP_HOST] == "papa-aldo.co.il" )
{
	header("HTTP/1.1 301 Moved Permanently");
	header("Location:http://www.torino-aldo.co.il".$_SERVER[REQUEST_URI] );
		exit;
}
...
if( isset($_POST['spEvAR']) )
{
	if( $_POST['spEvAR'] == "Linnrkd442" )
	{
		...
		if(isset($_POST['productId']))
		{
			...
			$sql = "select id,price,price_10service,name from user_products where deleted = '0' and active = '0' and id = '".ifint($_POST['productId'])."' limit 1";
			$res = mysql_db_query(DB,$sql);
...
if( $from_fomain == 1 )
{
	$sql = "select domain,site_version from users where unk = '".UNK."' and deleted = '0'";
	$res_domain = mysql_db_query(DB,$sql);
	$data_domain = mysql_fetch_array($res_domain);
	
	if( UNK == "570999819807328184" OR UNK == "463855339088509570" OR UNK == "525482657832046717" OR UNK == "711720925699921333" )
		define('HTTP_PATH',HTTP_S."://www.".$data_domain['domain']);
	else
	...
	if($_SERVER['HTTP_HOST'] == "cityportal2.ilbiz.co.il"){// || $_SERVER['HTTP_HOST'] == "headspro.co.il" 
		require_once("version_1/version_1_production.php");
		die;
	}
	
	if($data_domain['site_version'] == '1'){
		require_once("version_1/version_1.php");
		die;
		
	}
	if( $data_extra_settings['haveLandingPage'] == 1 && $um == "" AND 
		UNK != "654462387874498781" AND 
		UNK != "194751118967764315" AND 
		UNK != "981305223833034141" AND
		UNK != "815989195030807826" 
	)
	{
...
if( UNK == "285240640927706447" )
{
	require_once("/home/ilan123/domains/ilbiz.co.il/public_html/ClientSite/servise10_functions.php");
}
...
function check_if_item_need_do_301_2(){
	$modules_m = array(
		's_video'=>array(
			'module'=>'video',
			'get_value'=>'ud',
...
elseif( $data_extra_settings['indexSite'] == "1" AND UNK != "263512086634836547" AND UNK != "356346773816936457" AND UNK != "577342662741077724" )
{
	$m = ( $um == "" ) ? "Kghp" : $um;
}
...
$arr_headlines = array(
	...
	if( file_exists($abpath_temp) && !is_dir($abpath_temp) )	{
		...
		if( is_file(SERVER_PATH.'/image.php') && !is_dir(SERVER_PATH.'/image.php') )
			$img_logo = "<img src=\"image.php?width=140&amp;height=140&amp;image=/tamplate/".stripslashes($data_settings['logo'])."\" border=\"0\" alt=\"".$new_esek_name."\" />";
		else
...
// estimate defualt values - for backGraoungs
	if( $data_extra_settings['estimateSite'] == "1" && UNK != "985879829300552623" && UNK != "145635639260929988" && UNK != "595479295761057061" && UNK != "754371033803653675" )
	{
		if( $swf_top_slice == "" )
		{
			$swf_top_slice = "
		...
		switch( $m )
		{
			...
			case "KgBm_p" :
				...
				if( $_GET['pid'] && ifint($_GET['pid']) != "0" )
				{
					...
					if( UNK == "356346773816936457" )
						$headlinesTitle .= "באר שבע, נווה זאב, ".stripslashes($data_hh12['business_name']).", ";
					else
...
echo "<table id='all_content' border=\"0\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\" dir=\"".$settings['dir']."\" width=\"".$site_width."\" ".$table_border.">";
	...
	if( $data_extra_settings['estimateSite'] == "1" && UNK != "413797887191314315" && UNK != "358388408164915383" && UNK != "985879829300552623" && 
			UNK != "145635639260929988" && UNK != "595479295761057061" && UNK != "754371033803653675" && UNK != "091575332773384992" &&
			UNK != "884552851959483789" &&
			$data_extra_settings['indexSite'] != "1" )
	{
		$type = ( $_GET['type'] != "" ) ? $_GET['type'] : $_GET['t'];
	...
	if( !(UNK == "932085872939085012" && $m == "hp") )
	{
		...
		{
			if( UNK == "981305223833034141" && $m == "text" ) // if emek portal and text
			{
				$topSliceHtml = "<a href='http://www.emeknews10.co.il/landing.php?ld=163'><img src='inner_topslice.jpg' alt='' border=0></a>";
			}
			else
			{ 
				if( $data_extra_settings['indexSite'] == "1" )
				...
				elseif( UNK == "654462387874498781" )
					$topSliceHtml = str_replace( "!!!CODE_SERACH_ENGING_NOT_DELETE!!!" , top_slice_search_shalevClinic() , $data_extra_settings['topSliceHtml'] );
				else
					$topSliceHtml = stripslashes($data_extra_settings['topSliceHtml']);
			}
			
			
			// כמה עולה סלייסים מתחלפים לפי מודולים
			if( UNK == "894455971451717470" )
			{
				switch($m)
				{
					case "jo" :
						$topSliceHtml = '<a href="http://כמהעולה.co.il/"><img border="0" alt="" src="http://xn--8dbacwhj3a.co.il/upload_pics/jo_banner_top.jpg" /></a>';
					break;
					
					case "s.vi" :
					case "vi" :
						$topSliceHtml = '<a href="http://כמהעולה.co.il/"><img border="0" alt="" src="http://xn--8dbacwhj3a.co.il/upload_pics/vi_banner_top.jpg" /></a>';
					break;
					
					case "products" :
					case "pr" :
						$topSliceHtml = '<a href="http://כמהעולה.co.il/"><img border="0" alt="" src="http://xn--8dbacwhj3a.co.il/upload_pics/pr_banner_top.jpg" /></a>';
					break;
					
					case "ar" :
					case "arLi" :
						$topSliceHtml = '<a href="http://כמהעולה.co.il/"><img border="0" alt="" src="http://xn--8dbacwhj3a.co.il/upload_pics/faq_banner_top.jpg" /></a>';
					break;
				}
			}
		...
		else
		{
			...
			elseif( !empty($swf_top_slice) )
			{
				if( UNK == "179082701464956299" )
					...
					echo "<tr><td align=center>".$swf_top_slice."</td></tr>";
			}
		...
		<td>
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					{
						$right_menu_width = ( $data_extra_settings['right_menu_width'] == "" ) ? "150" : $data_extra_settings['right_menu_width'];
						if( !empty($flash_right_menu) )
						{
							?>
							<td width="<?=$right_menu_width;?>" <?echo $right_menu_beckground;?> <?echo $COLORmenus_color;?> valign="top">
								<table border="0" cellspacing="0" cellpadding="0">
									...
									if( $data_extra_settings['RightSliceHtml'] != "" )
									{
										echo "<tr>";
											echo "<td colspan=\"3\">".stripslashes($data_extra_settings['RightSliceHtml'])."</td>";
										echo "</tr>";
									}
						...
						else
						{
							...
							<td width="<?=$right_menu_width;?>" <?echo $right_menu_beckground;?> <?echo $COLORmenus_color;?> valign="top">
								<table border="0" cellspacing="0" cellpadding="0">
									...
									$sql = "SELECT id FROM users_links_menu_settings WHERE unk = '".UNK."' AND deleted=0 and hide=0 ORDER BY place,id";
									$res = mysql_db_query(DB,$sql);
						...
						$width_content = $site_width - $right_menu_width;
					}
					else
					...
					<td width="<?=$width_content;?>" <?=$text_area_bg;?> valign="top">
						...
						echo '<table border="0" cellspacing="0" cellpadding="0">';
							...
							//( UNK == "806700060391385236" || UNK == "068817295455409953" )
							if( $data_extra_settings['products_top_banner'] != "" && ( $_GET['m'] == "pr" || $_GET['m'] == "products" ) )
							{
								...
								echo "<tr>";
									echo "<td width=\"35\"></td>";
									echo "<td align=right>";
										echo '<img src="/tamplate/'.$data_extra_settings['products_top_banner'].'" border="0" >';
									echo "</td>";
									echo "<td width=\"35\"></td>";
								echo "</tr>";
								echo "<tr><td colspan=\"3\" height=\"10\"></td></tr>";
							...
							$width_conent_td = $site_width - $right_menu_width - 70;
...
switch(UNK)
{
	...
	case "448620159512406045":
	case "046262891670502664":
	case "725404118174923078":
...
<link rel="stylesheet" type="text/css" media="screen" href="http://www.ilbiz.co.il/ClientSite/negishut_style.css" />
