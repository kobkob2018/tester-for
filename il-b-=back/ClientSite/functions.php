...
function get_text_area($type="")	{
	...
	echo "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"maintext\">";
		...
		{
			$sql = "SELECT id FROM user_design_page_cat WHERE page_id = '".ifint($type)."' AND active=0 AND deleted=0 ORDER BY id LIMIT 1";
			$res = mysql_db_query(DB ,$sql );
			$first_c = mysql_fetch_array($res);
			
			$c = ( $_GET['c'] == "" ) ? $first_c['id'] : $_GET['c'];
			
			$sql = "SELECT id FROM user_design_page WHERE cat = '".ifint($c)."' ORDER BY id LIMIT 1";
	    ...
	    $first_dd = mysql_fetch_array($res2);
			...
			$sql = "SELECT id, title, content FROM user_design_page WHERE cat = '".ifint($c)."' and id = '".ifint($dd)."'";
	    $res2 = mysql_db_query(DB ,$sql );
		...
		if( UNK == "358388408164915383" && $_SERVER[REQUEST_URI] == "/" )
			$content = str_replace( "HERE_WE_HAVE_ESTIMATE_FORM__DO_NOT_DELETE" , ilbiz_side_from_hp() , $content );
		$str_content_view = "<tr>";
		...
		$view_it = 0;
				...
				if( $data_extra_settings['estimateSite'] == "1" )
				{
					...
					{
						$sql = "select id , google_adsense  from estimate_miniSite_defualt_block where unk = '".UNK."' and type = '".$type."'";
						$res = mysql_db_query(DB,$sql);
						$data_estimate = mysql_fetch_array($res);
						
						$not_pages = array( "9471","9470","9468","text","9472","9473","9474","9475","9476",
						"9477","9478","9479","9480","9481","9482");
						
						if( !in_array( $type , $not_pages) )
						{
							
							if( empty($data_estimate['id']) || $data_estimate['google_adsense'] == "0" )
							{
...
google_ad_client = "pub-5037108574233722";
/* EstimatePages */
google_ad_slot = "6492531233";
google_ad_width = 468;
google_ad_height = 60;
//-->
</script>
...
';
				...
				if( UNK == "285240640927706447" )
				{
					$sql = "select primeryCat , subCat , choosenClientId from estimate_miniSite_defualt_block where unk = '".UNK."' and type = '".$type."'";
					$res = mysql_db_query(DB,$sql);
					...
					if( $data_estimate2['choosenClientId'] == "0" )
					{
						echo "<tr>";
							echo "<td>";
								echo get_buttom_clients_cats( $data_estimate2['primeryCat'] , $data_estimate2['subCat'] );
							echo "</td>";
						echo "</tr>";
					}
...
function jobs()
{
	...
	$sql = "select uw.* from user_wanted as uw LEFT JOIN user_wanted_cats as uc ON uw.id=uc.wanted_id where uw.unk = '".UNK."' ".$where." and uw.deleted = '0' GROUP BY uw.id order by uw.place";
	$res = mysql_db_query(DB,$sql);
...
{
	global $word,$data_colors,$data_name;
...
$total_price_to_pay = 0;
	...
	echo "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"maintext\">";
		...
		{
			echo "<tr>";
				echo "<td><iframe src ='http://www.ilbiz.co.il/newsite/net_system/BuyEcom.php?unk=".UNK."&ecomSesid=".$_SESSION['ecom']['unickSES']."' width='535' frameborder=0 height=250 scrolling=auto allowtransparency='true'></iframe></td>";
			echo "</tr>";
		}
		else
		{
