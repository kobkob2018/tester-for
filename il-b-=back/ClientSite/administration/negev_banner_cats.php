...
<head>
	...
	<title>..::  באנרים - הוספה/עדכון קטגוריות  ::..</title>
...
<?
	...
	$sql = "select cat_name,id from user_guide_cats where father = 0 and deleted = 0 and unk='".UNK."' ORDER BY cat_name";
		...
		$cat_list .= "<tr>";
			$cat_list .= "<td style=\"background-color:#cccccc;\">";
				...
				$cat_list .= "<ul style=\"list-style:none\" class=\"dhtmlTree\">";
					...
					{
						$sql = "select cat_id from banner_choosen_biz_cat where banner_id = '".$_GET['banner_id']."' and cat_id = '".$data_father['id']."'";
						...
						$sql = "select cat_name,id from user_guide_cats where father = ".$data_father['id']." and unk='".UNK."' and deleted=0 ORDER BY cat_name";
						...
						$cat_list .= "<ul id=\"treeCAT".$data_father['id']."\" htmldb:listlevel=\"2\" style=\"padding-right:15px;list-style:none;display:none;\" class=\"dhtmlTree\">";
							...
							{
								$sql = "select cat_id from banner_choosen_biz_cat where banner_id = '".$_GET['banner_id']."' and cat_id = '".$data_father_cat['id']."'";
								$res_cat_id = mysql_db_query(DB,$sql);
								$data_cat_id = mysql_fetch_array($res_cat_id);
	...
	$form_arr = array(
		array("hidden","main","BannerSetCat"),
		array("hidden","banner_id",$_GET['banner_id']),
