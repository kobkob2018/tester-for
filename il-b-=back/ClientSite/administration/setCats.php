...
<?
	...
	$sql = "select id from user_wanted where unk = '".UNK."' and id = '".$_GET['wid']."'";
	$res_wanted = mysql_db_query(DB,$sql);
	$data_wanted = mysql_fetch_array($res_wanted);
	$wanted_id = $data_wanted['id'];
	
	$sql = "select cat_name,id from biz_categories where father = 0 and status = 1 and hidden=0";
			...
			$cat_list .= "<td style=\"background-color:#cccccc;\">";
				...
				$cat_list .= "<ul style=\"list-style:none\" class=\"dhtmlTree\">";
					while( $data_father = mysql_fetch_array($res_father) )
					{
						...
						$sql = "select cat_id from user_wanted_cats where wanted_id = '".$wanted_id."' and cat_id = '".$data_father['id']."'";
						...
						$cat_list .= "<ul id=\"treeCAT".$data_father['id']."\" htmldb:listlevel=\"2\" style=\"padding-right:15px;list-style:none;display:none;\" class=\"dhtmlTree\">";
							...
							{
								$sql = "select cat_id from user_wanted_cats where wanted_id = '".$wanted_id."' and cat_id = '".$data_father_cat['id']."'";
								$res_cat_id = mysql_db_query(DB,$sql);
								$data_cat_id = mysql_fetch_array($res_cat_id);
								...
								if( $num_father_tat_cat > 0 )
								{
									...
									{
										$sql = "select cat_id from user_wanted_cats where wanted_id = '".$wanted_id."' and cat_id = '".$data_father_tat_cat['id']."'";
										$res_cat_id = mysql_db_query(DB,$sql);
										$data_cat_id = mysql_fetch_array($res_cat_id);
	...
	$form_arr = array(
		array("hidden","main","funcSetCatForUsers"),
		array("hidden","wanted_id",$wanted_id),
