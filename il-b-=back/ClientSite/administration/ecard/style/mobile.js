		  ...
		  data: search_data,
			...
			if(data['success'] != '1'){
				...
				$.each(data['r_data'], function(i, item) {
					...
					try{
						row_clone.find(".subject").html(item['subject'].split("\\").join(""));
					}
					catch(err) {
		...
		$.ajax({
		  ...
		  data: send_data,
			...
			if(return_data['success'] == '0'){
				...
				lead_clone.find(".subject").val(item['subject'].replace("\\",""));
				...
				$.ajax({
				  ...
				  data: {	'user': $("#login_user").val(),
					...
					if(return_data['success'] == '0'){
						...
						var return_screen = $("#login_return_screen").val();
						if(return_screen == 'lead_list'){
							get_lead_list();
						}
						else{
							switch_screens(return_screen,return_data);
						}
					}
				}).error(function(){
					sudjest_refresh_page();
...
var edit_card_iframe_height_set = false;
function setup_edit_card(){
	jQuery(function($){
		$("#edit_card").click(function(){
			restart_edit_card_iframe();
		}); 
		
	});	
}

function restart_edit_card_iframe(){
	switch_screens('edit_card',new Array());
	if(!edit_card_iframe_height_set){
		$("#edit_card_iframe").height($(window).height());
		edit_card_iframe_height_set = true;
	}
	//document.location = "/myleads/?main=buy_leads&content_type=iframe&sessid="+$("#sessid_holder").val();
	$("#edit_card_iframe").attr('src', "/mycard/?main=module&c=edit_card&f=edit_card_form&content_type=iframe&sessid="+$("#sessid_holder").val());
}

function handle_edit_card_error(messege){
	system_messege(messege);
	setTimeout(function(){restart_edit_card_iframe();},'2000');
}

function handle_edit_card_success(messege){
	system_messege(messege);
...
jQuery(function($){
	$(document).ready(function(){
		...
		else if(start_screen == 'main_menu'){
			var user_loged = $("#main_content").attr("data-user_loged");
			if(user_loged != '1'){
				$("#login_return_screen").val(start_screen);
				console.log("user_not_loged"+user_loged);
				open_login_form();
			}
			else{
				switch_screens(start_screen,null);
			}
		}
		else{
