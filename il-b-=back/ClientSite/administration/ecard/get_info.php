...
if(isset($_REQUEST['row_id']) && isset($_REQUEST['token'])){
	...
	$sql = "select unk from user_e_card_forms where 
	...
	if(isset($data['unk']) && $data['unk'] != ""){
		...
		//remove token
		$sql = "update user_e_card_forms SET auth_token  = '' where 
				id = '".$_REQUEST['row_id']."' and 
				auth_token = '".$_REQUEST['token']."'";
		$res = mysql_db_query(DB,$sql);		
	}
...
require_once('ecard/get_content.php');	

$main = ( $_REQUEST['main'] == "" ) ? "menu" : $_REQUEST['main'];

$return_info['type'] = $main;
if($main == "module" && $_REQUEST['c'] == "reg_token"){
	get_content($main);
}
elseif($return_info['login'] == '1' || $main == 'user_login' ||  $main == 'forgot_password' ){
...
if(isset($_REQUEST['content_type']) && $_REQUEST['content_type'] == 'iframe'){
	require_once('ecard/iframe_template.php');	
}
else{
