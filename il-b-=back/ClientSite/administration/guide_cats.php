...
<head>
	...
	<title>..::  מדריכים - הוספה/עדכון קטגוריות  ::..</title>
...
<?
		...
		$cat_list .= "<tr>";
			$cat_list .= "<td style=\"background-color:#cccccc;\">";
				...
				$cat_list .= "<ul style=\"list-style:none\" class=\"dhtmlTree\">";
					...
					{
						$sql = "select cat_id from user_guide_choosen_biz_cat where biz_id = '".$_GET['biz_id']."' and cat_id = '".$data_father['id']."'";
						...
						$cat_list .= "<ul id=\"treeCAT".$data_father['id']."\" htmldb:listlevel=\"2\" style=\"padding-right:15px;list-style:none;display:none;\" class=\"dhtmlTree\">";
							...
							{
								$sql = "select cat_id from user_guide_choosen_biz_cat where biz_id = '".$_GET['biz_id']."' and cat_id = '".$data_father_cat['id']."'";
								$res_cat_id = mysql_db_query(DB,$sql);
								$data_cat_id = mysql_fetch_array($res_cat_id);
	...
	$form_arr = array(
		array("hidden","main","GuideSetCat"),
		array("hidden","biz_id",$_GET['biz_id']),
