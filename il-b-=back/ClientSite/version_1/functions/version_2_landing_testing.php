		...
		<body>
			<?php
				...
				if( file_exists($logo_abpath_temp1) && !is_dir($logo_abpath_temp1) )
				{
					...
					$logo = '<img class="lazy" data-src="'.$logo_url.'" title="'.$addMoreTitle.'" />';
				...
				if( file_exists($abpath_temp1) && !is_dir($abpath_temp1) )
				{
					...
					$top_slice = '<img class="lazy" data-src="'.$top_slice_url.'" title="'.$addMoreTitle.'" />';
				...
				{
					$mobile_top_slice_url = HTTP_PATH."/new_images/landing/".$dataLDSetting['topslice_bg_mobile'];
					$mobile_top_slice = '<img class="lazy" style="width:100%;" data-src="'.$mobile_top_slice_url.'" title="'.$addMoreTitle.'" />';
					...
					$module_index = 0;
						while( $dataModules = mysql_fetch_array($resModules) )
						{
							...
							if( file_exists($abpath_temp2) && !is_dir($abpath_temp2) )
								$titeIcon = "<img class='lazy' data-src=\"".HTTP_PATH."/new_images/landing/".$dataModules['title_icon']."\" border=0 style='padding-left: 20px;'>";
							else
								$titeIcon = "";
													...
													while( $dataParagraph = mysql_fetch_array($resParagraph) )
													{
														...
														switch( $dataParagraph['module_type'] )
														{
															case "0" :
																...
																echo "<tr>";
																	$change_youtube = str_replace( 'src="https://www.youtube.com/embed' , 'class="lazy" data-src="https://www.youtube.com/embed' , stripslashes($dataParagraph['p_text']) );
																	echo "<td>".$change_youtube."</td>";
																echo "</tr>";
																echo "<tr><td height=7></td></tr>";
															break;
																			...
																			$abpath_temp = SERVER_PATH."/video/".$dataVid['img'];
																				...
																				echo "<div class='landing_text small-vid'>";
																					if( file_exists($abpath_temp) && !is_dir($abpath_temp) )
																						echo "<img class='lazy' data-src='video/".$dataVid['img']."' border='0' style='border: 1px solid #".$dataModules['title_color'].";'>";
																					...
																					//if( $counter%4 == 0 ) 
																						echo "<div class='galery-img'>";
																								...
																								if( file_exists($abpath_temp_unlink) && !is_dir($abpath_temp_unlink) )
																								{
																									...
																									echo "<a href=\"".HTTP_PATH."/gallery/L".$dataImg['img']."\"  rel=\"fancy_image\" ".$content." ><img class='lazy' data-src=\"".HTTP_PATH."/gallery/".$dataImg['img']."\" rel=\"lightbox\" border=\"0\"></a>";
																								}
																							echo "</div>";
