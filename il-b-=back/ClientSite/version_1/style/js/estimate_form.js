	...
	function setup_estimate_form_extra_fields(cat_input_id){
		jQuery(function($){
			...
			if($("#"+cat_input_id).length != 0){
				...
				if($("#estimate_form").find("#in_cities").length != 0){
					var in_cities = $("#estimate_form").find("#in_cities").val();
					params += "&limit_cat_by_cities="+in_cities;
				}				
				
				var ef_url = "ajax.php?main=estimateCatExtraFileds&version=version_1" + params ;
				jQuery.ajax({
					url: ef_url,
					type: 'GET',
					success: function (transport) {
						...
						$("#estimate_extra_fields_helper").find(".Fm_email_temp").each(function(){
							if($("#Fm_email").length == 0 && $("#estimate_form .Fm_email_temp").length == 0){
								$(this).insertBefore( "#estimate_form #Fm_note_wrap" );
							}	
						});	
						console.log($("#estimate_extra_fields_helper .Fm_email_remove").length);
						if($("#estimate_extra_fields_helper .Fm_email_remove").length != 0){
							$("#estimate_form").find(".Fm_email_temp").each(function(){
									$(this).remove();
							});
						}						
	...
	function ajax_estimateForm_send()
	{
		...
		var ef_url = "ajax.php?main=estimateSiteInsert&add_wach_link=1&" + queryString ;
		jQuery.ajax({
			url: ef_url,
			type: 'GET',
			success: function (transport) {
				document.getElementById("estimateSiteHeightDiv").innerHTML = transport;
				if(jQuery("#tracking_form_id_holder").length != 0){
					var estimateFormId = jQuery("#tracking_form_id_holder").val();
					var customer_send = jQuery("#tracking_form_customer_send_holder").val(); 
					console.log(estimateFormId);
					if(typeof tracking_form_send == 'function'){
						tracking_form_send(estimateFormId,customer_send);
					}
					else{
						console.log('f no fnd');
					}
				}
				if ( jQuery( "#watting_redirect_form" ).length !== 0 ) { 
					setTimeout(function(){jQuery("#watting_redirect_form").submit();},1000);
				}
			},
			error: function () {
				console.log("err:c_c_f.p(1250)");
	...
	function estimate_form_auto_popup(){
		...
		if(popup_do){
			setTimeout(function(){estimate_form_button_click();},30000);
		}
		jQuery(function($){
	...
	function estimate_form_button_blink(){
		jQuery(function($){
			setInterval(function(){
				if($("#mobile_estimate_form_open_button").find("a").attr("rel") == "closed"){
					$("#mobile_estimate_form_open_button a").css("background-color","#8e8e8e");
					
					setTimeout(function(){
						$("#mobile_estimate_form_open_button a").css("background-color","");						
					},200);
				}	
