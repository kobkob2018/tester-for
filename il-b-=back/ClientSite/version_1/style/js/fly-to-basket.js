...
var txt_totalPrice = get_word_txt_totalPrice();
var txt_COIN_type = get_word_COIN_type();
...
function addToBasket(productId  )
{
	...
	button_content = document.getElementById('addToBasketButton' + productId).innerHTML;
	document.getElementById('addToBasketButton' + productId).innerHTML = "<b><u>" + added_to_cart() + "</u></b>";
	setTimeout(function(){return_addtocart_button(productId,button_content)},2000);
...
function return_addtocart_button( productId,content )
{
	document.getElementById('addToBasketButton' + productId).innerHTML = content;
}
...
function showAjaxBasketContent(ajaxIndex)
{
	...
	if(document.getElementById('shopping_cart_items_product' + productItems[0])){	// A product with this id is allready in the basket - just add number items
		...
		img.src = 'http://ilbiz.co.il/ClientSite/other/sym_img/remove.gif';
		a.appendChild(img);
...
jQuery(function($){
	$("#shopping_cart_header").click(function(){
		if($(this).attr("rel") == 'close'){
			$(this).attr("rel","open");
			$(".hidden-cart-part").show();
		}
		else{
			$(this).attr("rel","close");
			$(".hidden-cart-part").hide();
		}
	});
});
...
function open_cart(){
	jQuery(function($){
		if($("#shopping_cart_header").attr("rel")=='close'){
			$("#shopping_cart_header").click();
		}
	});
}