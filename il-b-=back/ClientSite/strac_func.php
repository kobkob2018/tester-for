...
function search()
{
			...
			$sql = "select * from {$val} where deleted = '0' and unk = '".UNK."' and ( ";
				...
				foreach ( $key as $kkey )
				{
					...
					switch($kkey)
					{
						...
						default:
							$sql .= "{$or_parametr} {$kkey} like '%".mysql_real_escape_string($_GET['search_val'])."%' ";
					}
					$counter++;
				}
			$sql .= ") ";
				...
				while( $data = mysql_fetch_array($res))
				{
					...
					switch($val2)
					{
						...
						case "user_content_pages" :
							...
							echo "<a href='index.php?m=text&t=".$data['type']."' class='maintext'><b><u>".nl2br(htmlspecialchars(stripslashes($data['name'])))."</u></b></a><br>";
							echo "<br><br>";
							$count_search++;
						break;
	...
	$sql = "select landing_id from sites_landingPage_modules where deleted = '0' AND visibility_hidden = '0' and unk = '".UNK."' and ( module_title like '%".$_GET['search_val']."%')";
	$res = mysql_db_query(DB,$sql);
	while( $data = mysql_fetch_array($res)){
	...
	$sql = "select slp.landing_id from sites_landingPage_paragraph slp LEFT JOIN sites_landingPage_modules slm ON slp.module_id = slm.id 
			where slp.deleted = '0' AND  slm.deleted = '0' AND slm.visibility_hidden = '0' and slp.unk = '".UNK."' and ( slp.p_text like '%".$_GET['search_val']."%')";
	$res = mysql_db_query(DB,$sql);
	while( $data = mysql_fetch_array($res)){
		$landing_ids[$data['landing_id']] = $data['landing_id'];
	}
	...
	$sql = "select * from sites_landingPage_settings where deleted = '0' and unk = '".UNK."' and ( landing_name like '%".$_GET['search_val']."%' or text_1 like '%".$_GET['search_val']."%' or text_2 like '%".$_GET['search_val']."%' ".$landing_sql_in.")";
...
function bottom_copyright()
{
	...
	echo "<table border=0 class=\"headline\" style=\"font-size:10px;\" width=\"100%\">";
		echo "<tr>";
			echo "<td align=\"".$settings['align']."\">".$word[LANG]['1_3_copyright']." ".trim(stripslashes($data_name['name']))."</td>";
			echo "<td align=\"".$settings['re_align']."\">";
			
			if( UNK == "688525412924909380" )
			{
				echo "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"headline\" style=\"font-size:10px;\">";
					echo "<tr>";
						echo "<td width=20></td>";
						echo "<td><a href='http://".$_SERVER['HTTP_HOST']."/index.php?m=contact' class='headline' style='font-size:10px;' title='צור קשר'>צור קשר</a></td>";
						echo "<td width=20></td>";
					echo "</tr>";
				echo "</table>";
			}
			...
			if( $data_extra_settings['estimateSite'] == "1" )
			{
				echo "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"headline\" style=\"font-size:10px;\">";
					echo "<tr>";
						if( UNK == "413797887191314315" )
						{
							echo "<td><a href='http://".$_SERVER['HTTP_HOST']."/index.php?m=contact' class='headline' style='font-size:10px;' title='צור קשר'>צור קשר</a></td>";
							echo "<td width=20></td>";
						}
						if( UNK != "894266530087576453" )
						{
							echo "<td><a href='http://".$_SERVER['HTTP_HOST']."/index.php?m=search' class='headline' style='font-size:10px;' title='מפת אתר'>מפת אתר</a></td>";
							echo "<td width=20></td>";
						}
						//echo "<td><a href='http://".$_SERVER['HTTP_HOST']."' class='headline' style='font-size:10px;' title='".trim(stripslashes($data_name['name']))."' >".trim(stripslashes($data_name['name'])). "</a></td>";
						echo "<td>".trim(stripslashes($data_name['name'])). "</td>";
					echo "</tr>";
				echo "</table>";
			}
			else
			{
				echo "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"headline\" style=\"font-size:10px;\">";
					echo "<tr>";
						...
						/*elseif( !empty($data_il['id']) )
						{
							echo "<td width=10></td>";
							echo "<td><a href=\"http://www.il-biz.com/\" class=\"headline\" style=\"font-size:10px;\" target=\"_blank\" title='פרסום באינטרנט'><u>פרסום באינטרנט</u></a></td>";
						}*/
						
						/*if( $data_name['have_69'] == "0" )
						{
							echo "<td width=10></td>";
							echo "<td>".$word[LANG]['1_3_copyright_ilbiz_slogen']." <a href=\"".$word[LANG]['1_3_copyright_ilbiz_site']."\" class=\"headline\" style=\"font-size:10px;\" target=\"_blank\" title='".$word[LANG]['1_3_copyright_ilbiz_name']."'><u>".$word[LANG]['1_3_copyright_ilbiz_name']."</u></a></u></td>";
						}*/
...
function marketing_partners_logout(){
	echo "<div style='display:none'><iframe src ='http://www.ilbiz.co.il/newsite/net_system/frame_logout.php?unk=".UNK."'></iframe></div>";
}
...
echo "
<!-- Search Google -->
	<form method=\"get\" action=\"http://www.google.co.il/custom\" target=\"google_window\" style='padding:0; margin:0;'>
...
function top_user_address()
{
	...
	echo "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"maintext\">";
			echo "<tr>";
						...
						if( $data_name['phone'] )
						{
							echo "<td>".$word[LANG]['1_3_top_details_phone']." ".stripslashes($data_name['phone'])."</td>";
							echo "<td width=\"10\"></td>";
							echo "<td>|</td>";
							echo "<td width=\"10\"></td>";
						}
